...
class PAC2002(TireModelBase):
    def __init__(self):
        self.Name = "PAC2002"
        self.Description = (
            "An implementation of Pacejka 2002 as described in the First Editions of Tire"
    ...
    def __calculate_E_y(self, dfz, gamma_y, alpha_y):
        ...
        E_y = (
            ...
            * (1 - (self.PEY3 + self.PEY4 * gamma_y) * np.sign(alpha_y))
            * self.LEY
        )
        if E_y > 1.0:
            E_y = 1.0
    ...
    def __calculate_S_Hy(self, dfz, gamma_y, zeta0, zeta4):
        ...
        # 40
        S_Hy = (
            (self.PHY1 + self.PHY2 * dfz) * self.LHY
            + self.PHY3 * gamma_y * zeta0
            + zeta4
            - 1
        )
    ...
    def __calculate_S_Vy(self, state, dfz, gamma_y, zeta4):
        ...
        S_Vy = (
            state.FZ
            * (
                (self.PVY1 + self.PVY2 * dfz) * self.LVY
                + (self.PVY3 + self.PVY4 * dfz) * gamma_y
            )
            * self.LMUY
            * zeta4
        )
    ...
    def __calculate_B_yk(self, alpha):
        ...
        # 70
        B_yk = (
            self.RBY1 * math.cos(math.atan(self.RBY2 * (alpha - self.RBY3))) * self.LYKA
        )
    ...
    def __calculate_D_Vyk(self, state, dfz, gamma_y, alpha, gamma):
        ...
        D_Vyk = (
            ...
            * math.cos(math.atan(self.RVY4 * alpha))
        )
    ...
    def __calculate_mu_x(self, dfz, gamma_star):
        ...
        # 23
        mu_x = (
            (self.PDX1 + self.PDX2 * dfz) * (1.0 - self.PDX3 * gamma_x**2.0) * self.LMUX
    ...
    def __calculate_B_t(self, dfz, gamma_z):
        ...
        # 50
        B_t = (
            (self.QBZ1 + self.QBZ2 * dfz + self.QBZ3 * dfz**2)
            * (1 + self.QBZ4 * gamma_z + self.QBZ5 * abs(gamma_z))
            * self.LKY
            / self.LMUY
        )
    ...
    def __calculate_D_t(self, state, dfz, gamma_z, zeta5):
        ...
        D_t = (
            ...
            * (1 + self.QDZ3 * gamma_z + self.QDZ4 * gamma_z**2)
            * self.UNLOADED_RADIUS
            / self.FNOMIN
            * self.LTR
            * zeta5
        )
    ...
    def __calculate_E_t(self, dfz, gamma_z, alpha_t, B_t, C_t):
        ...
        # 53
        E_t = (self.QEZ1 + self.QEZ2 * dfz + self.QEZ3 * dfz**2) * (
            1
    ...
    def __calculate_D_r(self, state, dfz, gamma_z, zeta8):
        ...
        D_r = (
            ...
            * (
                ...
                + (self.QDZ8 + self.QDZ9 * dfz) * gamma_z
            )
            * self.UNLOADED_RADIUS
            * self.LMUY
            + zeta8
    ...
    def solve(self, input_state, mode=SolverMode.All):
        ...
        if (mode is SolverMode.PureFy) or (mode is SolverMode.PureMz):
            state.FY = self.calculate_pure_fy(input_state)

        if mode is SolverMode.Fy or mode is SolverMode.All:
            state.FY = self.calculate_fy(input_state)

        if mode is SolverMode.PureFx:
            state.FX = self.calculate_pure_fx(input_state)

        if mode is SolverMode.Fx or mode is SolverMode.All:
            state.FX = self.calculate_fx(input_state)

        if mode is SolverMode.PureMz:
            input_state.FY = self.calculate_pure_fy(input_state)
            state.MZ = self.calculate_pure_mz(input_state)

        if mode is SolverMode.Mz or mode is SolverMode.All:
            state.MZ = self.calculate_mz(input_state)

        if mode is SolverMode.Mx or mode is SolverMode.All:
            state.MX = self.calculate_mx(input_state)

        if mode is SolverMode.Mz or mode is SolverMode.All:
            state.MY = self.calculate_my(input_state)

        if mode is SolverMode.Radius or mode is SolverMode.All:
            state.RL, state.RE = self.calculate_radius(input_state)

        if mode is SolverMode.Relaxation or mode is SolverMode.All:
            state.SIGMA_ALPHA = self.calculate_lateral_relaxation_length(input_state)
            state.SIGMA_KAPPA = self.calculate_longitudinal_relaxation_length(
                input_state
            )
    ...
    def calculate_mz(self, state):
        ...
        s = (
            ...
            * (
                self.SSZ1
                + self.SSZ2 * (state.FY / (self.FNOMIN * self.LFZ0))
                + (self.SSZ3 + self.SSZ4 * dfz) * gamma_star
            )
            * self.LS
        )
    ...
    def calculate_mx(self, state):
        ...
        M_x = (
            ...
            * (
                self.QSX1 * self.LVMX
                - self.QSX2 * gamma_star
                + self.QSX3 * state.FY / self.FNOMIN
            )
            * self.LMX
        )
    ...
    def calculate_my(self, state):
        ...
        M_y = (
            ...
            * (
                self.QSY1
                + self.QSY2 * state.FX / self.FNOMIN
                + self.QSY3 * abs(state.V / self.LONGVL)
                + self.QSY4 * (state.V / self.LONGVL) ** 4
            )
            * self.LMY
        )
    ...
    def calculate_radius(self, state):
        ...
        R_omega = (
            ...
            + self.QV1
            * self.UNLOADED_RADIUS
            * (omega * self.UNLOADED_RADIUS / self.LONGVL) ** 2
        )
        ...
        C_z0 = (
            self.FNOMIN
            / self.UNLOADED_RADIUS
            * math.sqrt(self.QFZ1**2 + 4.0 * self.QFZ2)
        )
        if C_z0 == 0.0:
            return 0.0, 0.0

        # Eff. Roll. Radius #This is a newer version
        R_e_old = R_omega - (self.FNOMIN / C_z0) * (
            self.DREFF * math.atan(self.BREFF * state.FZ / self.FNOMIN)
            + self.FREFF * state.FZ / self.FNOMIN
        )
        ...
        R_e = R_omega - rho_Fz0 * (
            self.DREFF * math.atan(self.BREFF * rho_d) + self.FREFF * rho_d
        )
    ...
    def calculate_lateral_relaxation_length(self, state):
        ...
        sigma_alpha = (
            self.PTY1
            * math.sin(
                2.0 * math.atan(state.FZ / (self.PTY2 * self.FNOMIN * self.LFZ0))
            )
            * (1 - self.PKY3 * abs(gamma_y))
            * self.UNLOADED_RADIUS
            * self.LFZ0
            * self.LSGAL
        )
    ...
    def calculate_longitudinal_relaxation_length(self, state):
        ...
        sigma_kappa = (
            state.FZ
            * (self.PTX1 + self.PTX2 * dfz)
            * math.exp(-self.PTX3 * dfz)
            * (self.UNLOADED_RADIUS / self.FNOMIN)
            * self.LSGKP
        )
